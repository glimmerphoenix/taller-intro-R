...
- Asignación: <-, =. Cualquier variable solo puede tener un *único valor* en cada instante.
...
- Creación de vector mediante repetición de secuencia:
...
- Función **gl()**: generación de secuencias a partir de factores. 
...
# 1.12 Un poco de estadística con R

## Funciones resumen

```{r summaries-i, echo=TRUE}
summary(CO2)
```
...
## Tablas de contingencias (vars. cualitativas)

```{r contingency, echo=TRUE}
with(CO2, table(Type, Treatment))

## ftable() proporciona un formato compacto
ftable(UCBAdmissions)
```

## Tablas de contingencias (vars. cualitativas)

```{r contingency-margin, echo=TRUE}
## Tablas de frecuencias marginales
## margin = 2 o 3 para otras variables
margin.table(UCBAdmissions, margin = 1) 
```

## Tablas de contingencias (vars. cualitativas)

```{r contingency-perc,  echo=TRUE }
## Porcentajes sobre el total de de admitidos/excluidos 
## por Dpto.
...
```{r hist, echo=TRUE, fig.height=3.25, fig.width=4.5}
hist(CO2$uptake)
```

## Función de densidad de probabilidad

```{r density, echo=TRUE, fig.height=3.25, fig.width=4.5}
plot(density(CO2$uptake))
```

## Mejorando la representación gráfica

\begin{huge}
Demasiado "simples" estos gráficos, ¿no es así?
\end{huge}

\vspace{1cm}
- Vamos a mejorarlos un poco con el paquete `ggplot2`
...
```{r ggplot2-hist, echo=TRUE, message=FALSE, fig.height=2.5, fig.width=4}
ggplot(CO2, aes(x=uptake)) +
  geom_histogram(fill="white", colour="black") +
  ggtitle("Histograma de  CO2$uptake")
```

## Densidad de probabilidad con ggplot2
```{r ggplot2-density, echo=TRUE, message=FALSE, fig.height=2.5, fig.width=4}
ggplot(CO2, aes(x=uptake)) +
...
- Leer los datos del ficheero `Superstore_Sales.csv` en la variable `superstore_sales`.
- A partir de los datos de dicha variable:
    1. Calcular la tabla de contingencias entre la variable "Ship Mode" y la variable "Region".
    2. Obtener el resumen numérico de las variabls "Sales", "Unit Price" y "Shipping Cost".
    3. Representar la función de densidad de probabilidad de la variable "Shipping Cost".
    
**Nota importante**: Debido a los espacios en blanco en los nombres originales de las
columnas, se deben cambiar de nombre en el proceso las columnas afectadas (o leerlas
correctamente desde el comienzo).

## Ejercicio 2

- Observe el siguiente gráfico:
```{r scatter-hdi-cpi, echo=TRUE, eval=FALSE}
economist_data <- read.csv("C:/Users/Chus Algar Díaz/Documents/D/CursosFormacion/UCLM-2024/Intro_R/data/EconomistData.csv")
...
```{r scatter-hdi-cpi-graph, fig.height=7}
economist_data <- read.csv("C:/Users/Chus Algar Díaz/Documents/D/CursosFormacion/UCLM-2024/Intro_R/data/EconomistData.csv")
ggplot(economist_data, aes(x=CPI, y=HDI)) +
  geom_point() +
  geom_smooth(method="lm") +
  ggtitle("HDI vs. CPI en diferentes países")
```

## Ejercicio 2

1. ¿Parece existir algún tipo de conexión entre los valores de ambas variables? ¿Por qué?
2. ¿Es adecuado el ajuste lineal que propone la gráfica? Razona la respuesta.